...
- データ取得境界: 初期版では `ManualInputStockDataProvider` / `ExternalApiStockDataProvider` / `MockStockDataProvider` 相当を分離し、外部API用Providerは疑似レスポンス変換から段階的に追加する
...
- 評価時は有効な手入力評価データがある場合に手入力値を優先し、手入力評価データが未登録または全項目空欄の場合は外部API疑似データ、固定モック値の順にフォールバックします
...
- `ExternalApiStockDataProvider` は外部API連携に向けたスタブです。現時点ではネットワーク通信を行わず、疑似レスポンスが渡された場合のみ `StockSnapshot` へ変換します
- `CompositeStockDataProvider` は、有効な手入力評価データ、外部API疑似データ、固定モック値の順に `StockSnapshot` を探します
- 将来のデータ取得優先順位は、1. ユーザーの手入力評価データ、2. 外部API取得値、3. 開発用の固定モック値とします。Step 12時点では外部API取得値は疑似レスポンスのみです
