...
- ウォッチリスト画面では、InMemoryWatchlistRepository の監視銘柄を一覧表示し、銘柄詳細へ遷移できます
...
- ウォッチリストのRepository境界は作成済みですが、SwiftDataによる本格永続化は未実装です
- 銘柄詳細画面では、銘柄情報と確認メモの一覧を表示し、確認メモの追加、編集、削除ができます
- 確認メモは `InvestmentMemoRepository` 境界を通して扱い、現時点では `InMemoryInvestmentMemoRepository` で動作確認します
- 確認メモはタイトル空不可、本文は空でも保存できる仕様です
- 確認メモのSwiftDataによる本格永続化は未実装です
- 銘柄詳細画面では、ユーザー設定条件の一覧を表示し、条件の追加、編集、削除、有効/無効切り替えができます
- ユーザー設定条件は `AlertRuleRepository` 境界を通して扱い、現時点では `InMemoryAlertRuleRepository` で動作確認します
...
- 条件一致履歴は `AlertMatchHistoryRepository` 境界を通して扱い、現時点では `InMemoryAlertMatchHistoryRepository` で動作確認します
- 通知送信は未実装です
- ユーザー設定条件のSwiftDataによる本格永続化は未実装です
- 条件一致履歴のSwiftDataによる本格永続化は未実装です
