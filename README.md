...
- `ApiKeyProviding`、`JQuantsApiConfiguration`、`HTTPClient`、`JQuantsRequestBuilder` は、J-Quants実通信へ進む前の入口として用意しています
...
- APIキーや秘密情報はソースコード、README、テスト、GitHubに含めません。開発時はXcode Scheme環境変数、ローカル `.xcconfig`、`.env` などを候補にし、これらは `.gitignore` で除外します
