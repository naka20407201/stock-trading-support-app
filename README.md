...
- 評価時は手入力評価データがある場合に手入力値を優先し、手入力評価データが未登録の場合は固定モック値へフォールバックします
...
- `ManualInputStockDataProvider` は、保存済みの手入力評価データから `StockSnapshot` を生成します
- `FallbackStockDataProvider` は、手入力評価データがある場合は手入力値を優先し、未登録の場合は固定モック値を使います
