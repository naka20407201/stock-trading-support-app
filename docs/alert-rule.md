...
手入力評価データがある場合は手入力値を優先し、手入力評価データが未登録の場合は固定モック値を使います。外部APIやリアルタイム取得はまだ実装しません。
...
Step 11 では、`ManualInputStockDataProvider` と `FallbackStockDataProvider` を追加します。評価時は手入力評価データを優先し、未登録の場合に `MockStockDataProvider` の固定モック値へフォールバックします。外部API連携そのものは実装しません。外部APIを追加する場合も、APIレスポンスを直接 AlertRuleEvaluator に渡さず、DataProvider 相当の境界で StockSnapshot へ変換してから評価します。
