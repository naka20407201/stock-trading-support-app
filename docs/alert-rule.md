...
| 条件種別 | 指標 | 初期版での扱い |
| --- | --- | --- |
| 株価がしきい値より大きい / 以上 / 未満 / 以下 / 等しい / 等しくない | currentPrice | 対応 |
| PERがしきい値と比較条件に一致 | per | 対応 |
| PBRがしきい値と比較条件に一致 | pbr | 対応 |
| 出来高がしきい値と比較条件に一致 | volume | 対応 |
...
Step 11 時点では、currentPrice、per、pbr、volume を条件追加画面で選択できます。これらは手入力評価データまたは固定モック値から StockSnapshot に含められる場合に評価し、対象指標が未入力または欠損している場合は「評価できません」として扱います。

手入力評価データは、少なくとも1つの指標値が入力されている場合のみ有効です。全項目空欄の手入力評価データは有効な手入力評価データなしとして扱い、固定モック値へのフォールバックを妨げないようにします。

Step 12 時点では有効な手入力評価データを優先し、手入力評価データが未登録または全項目空欄の場合は外部API疑似データ、固定モック値の順で評価用データを探します。外部API疑似データはローカルに渡されたレスポンス相当DTOから `StockSnapshot` へ変換するだけで、実ネットワーク通信はまだ行いません。
...
7. 通知送信は後続対応とし、初期版では画面内の履歴表示にとどめる
...
Step 12 では、`ExternalStockSnapshotResponse` と `ExternalApiStockDataProvider` を追加します。`ExternalApiStockDataProvider` はネットワーク通信を行わず、疑似レスポンスから `StockSnapshot` を生成します。外部APIを追加する場合も、APIレスポンスを直接 AlertRuleEvaluator に渡さず、DataProvider 相当の境界で StockSnapshot へ変換してから評価します。APIキー未設定、取得失敗、レート制限、欠損値はDataProvider層で扱い、評価結果は必要に応じて「評価できません」として表示します。
...
外部API疑似データでも currentPrice、PER、PBR、出来高がすべて nil の場合は有効な `StockSnapshot` を作らず、次のProviderへフォールバックします。
