...
Step 11前半では、MockStockDataProvider が currentPrice、PER、PBR、出来高を StockSnapshot に含められる土台を用意します。ただし、条件追加画面の選択対象は引き続き currentPrice のみです。PER、PBR、出来高をUIで選択可能にする場合は、固定モック値、手入力値、将来の外部取得値の扱いを整理したうえで後続対応にします。
...
Step 11前半では、外部API連携そのものは実装しません。外部APIを追加する場合も、APIレスポンスを直接 AlertRuleEvaluator に渡さず、DataProvider 相当の境界で StockSnapshot へ変換してから評価します。
