...
Step 12.5 時点では、通常起動時は有効な手入力評価データを優先し、手入力評価データが未登録または全項目空欄の場合は固定モック値を使います。開発・テスト用では手入力評価データ、外部API疑似データ、固定モック値の順で評価用データを探します。外部API疑似データはローカルに渡されたレスポンス相当DTOから `StockSnapshot` へ変換するだけで、実ネットワーク通信はまだ行いません。
...
開発・テスト用では、2番目に `StubExternalStockDataProvider` の外部API疑似データを使います。外部API疑似データまたは外部API取得値でも currentPrice、PER、PBR、出来高がすべて nil の場合は有効な `StockSnapshot` を作らず、次のProviderへフォールバックします。
