...
Step 8 時点では、条件の登録、編集、削除、有効/無効切り替えに加えて、固定モック株価を使った条件評価と条件一致履歴作成までを実装しています。通知送信は後続Stepで扱います。
...
Step 8 では、同じ `snapshot.capturedAt` と条件IDの組み合わせでは重複履歴を作成しません。異なる取得時刻の Snapshot で再評価した場合は、同じ条件でも新しい条件一致履歴として扱えます。
