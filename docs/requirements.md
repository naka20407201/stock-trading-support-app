...
- 手入力評価データは少なくとも1つの指標値が入力されている場合のみ有効とし、全項目空欄の場合は有効な手入力評価データなしとして扱う
- 現時点の評価データ優先順位は、有効な手入力評価データ、固定モック値の順とする
- 将来の評価データ優先順位は、ユーザーの手入力評価データ、外部API取得値、開発用固定モック値の順とする
- 外部APIレスポンスは直接 AlertRuleEvaluator に渡さず、DataProvider相当の境界で StockSnapshot に変換する
...
- 有効な手入力評価データがある場合は手入力値を優先し、手入力評価データが未登録または全項目空欄の場合は固定モック値を使う
