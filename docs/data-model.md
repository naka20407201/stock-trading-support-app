...
- `StubExternalStockDataProvider` または将来実通信時の `ExternalApiStockDataProvider` が `ExternalStockSnapshotResponse` を `StockSnapshot` へ変換する。
...
- 外部API連携に向けたProviderは、Step 11 時点では設計候補に留め、ネットワーク通信は行わない
...
- Step 12 時点では外部API疑似データ用Providerがネットワーク通信を行わず、ローカルに渡された疑似レスポンスから `StockSnapshot` を返す
- `CompositeStockDataProvider` は複数の `StockDataProviding` を順番に試す
...
- 疑似レスポンスProviderを `StubExternalStockDataProvider` として分離する
- `ExternalApiStockDataProvider` は将来の `ExternalStockDataClient` を使う実通信Providerとして整理する
- `JQuantsStockDataClient` と `JQuantsStockDataMapper` を設計スタブとして追加する
- 通常起動時は空の外部API Provider を挟まず、`ManualInputStockDataProvider`、`MockStockDataProvider` の順で評価する
