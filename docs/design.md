...
Step 11前半では、SwiftData Repository の読み取り失敗時に `RepositoryReadStatusProviding` を通してViewModelへエラーメッセージを渡します。WatchlistView、確認メモ、ユーザー設定条件、条件評価では「未登録」と「読み込みに失敗」を区別できるようにし、Viewは引き続きSwiftDataの `ModelContext` を直接操作しません。
...
Step 11前半の時点では、`MockStockDataProvider` が固定モック値として currentPrice、PER、PBR、出来高を `StockSnapshot` に含められる土台を持ちます。ただし、条件追加画面で選択できる対象指標は引き続き currentPrice のみとし、PER、PBR、出来高のUI選択解放は外部データ取得や手入力値の扱いを整理した後に行います。
