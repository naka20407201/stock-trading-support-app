...
- ManualStockSnapshotInputEditorView
- ManualStockSnapshotInputViewModel
- SettingsView
...
永続化品質改善では、SwiftData Repository の読み取り失敗時に `RepositoryReadStatusProviding` を通してViewModelへエラーメッセージを渡します。WatchlistView、確認メモ、ユーザー設定条件、条件評価では「未登録」と「読み込みに失敗」を区別できるようにし、Viewは引き続きSwiftDataの `ModelContext` を直接操作しません。
...
対象指標と比較演算子は分離します。Step 11 時点では currentPrice、per、pbr、volume を手入力値またはモック値として評価できます。
